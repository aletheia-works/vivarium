...
<Page>
      ...
      items={[
        {
          ...
          body: <>Browse <a href="/vivarium/repro/">the gallery</a> and click through to the recipe page. Note the slug (e.g. <code><ExampleSlug layer={1} fallback="<recipe-slug>" /></code>).</>,
        },
        {
  ...
  >
    <p>Layer 1 recipes that opt into Path A render a <strong>"Try a fix"</strong> panel underneath the baseline output. No recipe ships the panel today — the runtime hook and the guide below stay in place for the next recipe that opts in.</p>
    <NumberedList
      items={[
        {
