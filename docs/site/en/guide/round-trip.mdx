...
<Page>
  ...
  >
    <NumberedList
      items={[
        ...
        {
          lead: 'Jujutsu (jj) on this side.',
          body: (
            <>
              Vivarium uses{' '}
              <a href="https://jj-vcs.github.io/jj/">Jujutsu</a> as its
              SCM rather than Git directly. The skill drives{' '}
              <code>jj describe</code> / <code>jj bookmark</code> /{' '}
              <code>jj git push</code> for the Vivarium-side PR and opens
              it with <code>gh pr create</code>.
            </>
          ),
        },
        {
  ...
  >
    <p>
      ...
      Layer 2 (Docker). Layer 3 ships no recipes, so it is out of scope
      for the fixed-verdict capture — see the cheat sheet below.
    </p>
    <pre>
...
Stage 4    Open the Vivarium PR (jj git push + gh pr create, label ai: generated)
...
Stage 7    Update Vivarium PR with fixed verdict
             (Layer 2 only; jj git push --bookmark)
...
Stage 9    Final Vivarium-side commit
             (Layer 2 only; jj git push --bookmark)`}</code>
    </pre>
  </Section>
  ...
  >
    ...
    <p>
      Layer 3: <code>verify_and_report_fix</code> rejects Layer 3{' '}
      <code>verify_fixed</code> with an informative error —{' '}
      <code>branch-fix-verdict.yml</code> only handles{' '}
      <code>src/layer2_docker/&lt;slug&gt;/</code>, and no Layer 3
      recipe exists to target.
    </p>
  </Section>
