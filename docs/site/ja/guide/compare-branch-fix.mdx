...
<Page>
  ...
  >
    ...
    <NumberedList
      items={[
        {
          lead: 'レシピが既に分かっている。',
          body: <><a href="/vivarium/ja/repro/">ギャラリー</a>を眺めて該当レシピのページに入る。slug をメモする（例: <code><ExampleSlug layer={1} fallback="<recipe-slug>" /></code>）。</>,
        },
        {
  ...
  >
    <p>Path A に opt-in した Layer 1 レシピは、ベースライン出力の下に{' '} <strong>「Try a fix」</strong> パネルを描画する。現時点でこのパネルを出荷しているレシピは無い。ランタイム側の仕組みとこのガイドは、次に opt-in するレシピのために残してある。</p>
    <NumberedList
      items={[
        {
          lead: 'レシピページを開く。',
