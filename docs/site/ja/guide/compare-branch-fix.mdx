...
<Page>
  ...
  >
    <p>Layer 2（Docker カタログ）はブラウザ タブでは走らせられない — バグは本物の OS、本物のソケット、 本物のファイルシステムを要求する。Path B はビルドをあなた自身の インフラに移し、push したイメージに対する verdict を GitHub Actions ワークフローでキャプチャする。</p>
    <NumberedList
      items={[
        {
