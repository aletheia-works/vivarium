  ...
  >
    <ul>
      ...
      <li>
        ...
        <code>mise run recipes:verify</code> の正確なコマンド、レシピ
        ディレクトリに置く placeholder の{' '}
        <code>recipe.json</code>（<code>docs/site/public/spec/recipe.schema.json</code>
        で検証される）、新規プロジェクト時のみ追加する
        <code>docs/site/_data/projects.json</code> の行、
