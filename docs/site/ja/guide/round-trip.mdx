...
<Page>
  ...
  >
    <NumberedList
      items={[
        ...
        {
          lead: 'Jujutsu (jj) がインストール済み。',
          body: (
            <>
              Vivarium は SCM として Git ではなく{' '}
              <a href="https://jj-vcs.github.io/jj/">Jujutsu</a>{' '}
              を使います。スキルは Vivarium 側 PR のために{' '}
              <code>jj describe</code> / <code>jj bookmark</code> / <code>jj git push</code>{' '}
              を駆動し、<code>gh pr create</code> で PR を開きます。
            </>
          ),
        },
        {
  ...
  >
    <p>
      Stage 0〜5 は Layer 非依存で共通、Stage 6 で Layer 1 (WASM in-browser) と Layer 2 (Docker) に分岐します。Layer 3 はレシピを持たないため fixed-verdict 取得の対象外 — 詳細は下の cheat sheet を参照。
    </p>
    <pre>
...
Stage 4    Vivarium PR open (jj git push + gh pr create、ai: generated ラベル付与)
...
Stage 7    Vivarium PR を fixed verdict で更新
             (Layer 2 のみ; jj git push --bookmark)
...
Stage 9    最終 Vivarium 側 commit
             (Layer 2 のみ; jj git push --bookmark)`}</code>
    </pre>
  </Section>
  ...
  >
    ...
    <p>
      ...
      <code>verify_fixed</code> が明示的に拒否されます — {' '}
      <code>branch-fix-verdict.yml</code> が{' '}
      <code>src/layer2_docker/&lt;slug&gt;/</code> しか扱えず、
      対象となる Layer 3 レシピも存在しないためです。
    </p>
  </Section>
