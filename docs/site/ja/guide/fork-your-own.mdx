...
<Page>
  ...
  >
    ...
    <p>Layer 1（ブラウザ内 WASM）は完全にあなたのフォーク側で完結します。 Layer 2（Docker）はアップストリームの GHCR イメージと統合された配信が前提なので、フォークでは <strong>Layer 1 のみ</strong> が deploy されます。</p>
    <Callout>
...
cd vivarium`}</code></pre>
  ...
  <Section
    ...
    heading="ラベル、Pages、main のルールセットを一括反映。"
  >
    <NumberedList
      items={[
        {
        ...
        {
          lead: 'plan して apply。',
          body: 'cd infra/github && tofu init && tofu plan の出力を確認した上で tofu apply。Pages の有効化、ラベル、main ブランチのルールセットがフォーク側に反映されます。',
        },
      ]}
    />
  </Section>
